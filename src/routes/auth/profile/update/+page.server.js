...
const profileUpdateSchema = object({
    ...
    qualifications: array().of(
        object({
            ...
            to: string().when('ongoing', { // Consider date type
                is: false,
                then: schema => schema.required("End date is required"),
                otherwise: schema => schema.nullable(),
            }),
...
export const load = async ({ locals, fetch }) => {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/profiles?filters[user][id][$eq]=${userId}&populate=*`, { // Added populate=* to get all fields
            headers: {
        ...
        if (!response.ok) {
            ...
            throw error(response.status, "Failed to fetch your profile data. Please try again later.");
        }
        ...
        if (err.status === 404 || err.status === 500) { // Propagate known errors
            throw err;
        }
        ...
        throw error(500, err.message || "An unexpected error occurred while loading your profile.");
    }
};
            ...
            qualifications: (() => {
                ...
                while (formData.has(`qualifications[${idx}].title`)) {
                    ...
                    quals.push({
                        ...
                        ongoing: formData.get(`qualifications[${idx}].ongoing`) === 'on' || formData.get(`qualifications[${idx}].ongoing`) === true, // ensure boolean
                    });
                    idx++;
                }
                return quals;
        ...
        try {
            ...
            if (!response.ok) {
                ...
                return fail(response.status, { 
                    ...
                    errors: responseData.error?.details?.errors || (validationError ? validationError.inner.reduce((acc, err) => {
                        const path = err.path.replace(/^val\./, '').replace(/\[(\d+)\]/g, '.$1');
